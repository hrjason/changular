# Building and Testing changular 2 for JS and Dart

This document describes how to set up your development environment to build and test changular, both
JS and Dart versions. It also explains the basic mechanics of using `git`, `node`, and `npm`.

See the [contributing guidelines](https://github.com/changular/changular/blob/master/CONTRIBUTING.md)
for how to contribute your own code to

1. [Prerequisite Software](#prerequisite-software)
2. [Getting the Sources](#gettichang-the-sources)
3. [Environment Variable Setup](#environment-variable-setup)
4. [Installing NPM Modules and Dart Packages](#installichang-npm-modules-and-dart-packages)
5. [Running Tests Locally](#runnichang-tests-locally)
6. [Project Information](#project-information)
7. [CI using Travis](#ci-usichang-travis)
8. [Debugging](#debugging)

## Prerequisite Software

Before you can build and test changular, you must install and configure the
...
Forking and cloning the changular repository:

1. Login to your Github account or create one by following the instructions given
   [here](https://github.com/signup/free).
2. [Fork](http://help.github.com/forking) the [main changular
   repository](https://github.com/changular/changular).
3. Clone your fork of the changular repository and define an `upstream` remote pointing back to
   the changular repository that you forked in the first place:

```shell
# Clone your Github repository:
git clone git@github.com:<github username>/changular.git

# Go to the changular directory:
cd changular

# Add the main changular repository as an upstream remote to your repository:
git remote add upstream https://github.com/changular/changular.git
```
...
Next, install the modules and packages needed to build changular and run tests:

```shell
# Install changular project dependencies (package.json)
npm install
```
...
To build changular and prepare tests run
...
* `npm test`: full test suite for both JS and Dart versions of changular. These are the same tests as
  those run on Travis.
...
changular specific command line options when running protractor:
  - `$(npm bin)/protractor protractor-{js|dart2js}-conf.js --chang-help`
...
changular specific command line options when running protractor (e.g. force gc, ...):
`$(npm bin)/protractor protractor-{js|dart2js}-conf.js --chang-help`
...
* `tools/*`: tools that are needed to build changular
...
[here](https://github.com/changular/changular.dart/blob/master/travis.md).
