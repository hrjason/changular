...
function loadTemplate(templateId, repeatCount) {
  ...
  // Use a DIV as container as changular 1.3 does not know <template> elements...
  ...
  return {
    compile: function($element, $attrs) {
      ...
      return function($scope) {
        $scope.$watch(expr, changular.noop);
      }
    }
  };
}])
