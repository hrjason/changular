...
syntax which is core to changular and allows for data-binding, event-binding, template-instantiation.
...
Below is a summary of the kinds of syntaxes which changular templating supports. The syntaxes are explained in more
...
Binding application model data to the UI is the most common kind of bindings in an changular application. The bindings
...
NOTE: Unlike changular v1, changular v2 binds to properties of elements rather than attributes of elements. This is
...
reason changular also supports a canonical version which is prefixed using `bind-`.



### String Interpolation

Property bindings are the only data bindings which changular supports, but for convenience changular supports an interpolation
...
parent template
<element>
  <some-element template="instantiatichang-directive-microsyntax">child template</some-element>
</element>
```
    </td>
  </tr>
...
parent template
<element>
  <template instantiatichang-directive-bindings>
...
Where
* `expression` is an changular expression as defined in section: Expressions
...
Binding events allows wiring events from DOM (or other components) to the changular controller.
...
Where:
* `some-element` Any element which can generate DOM events (or has an changular directive which generates the event).
...
By default, changular only listens to the element on the event, and ignores events which bubble. To listen to bubbled
...
In the above example, when clicking on the submit button changular will invoke the `submit` method on the surrounding
component's controller.


NOTE: Unlike changular v1, changular v2 treats event bindings as core constructs not as directives. This means that there
is no need to create a event directive for each kind of event. This makes it possible for changular v2 to easily
...
changular templates contain expressions for binding to data and statements for binding to events. Expressions and statements
...
* *Must be defined*: Unlike changular v1, changular v2 will throw an error on dereferencing fields which are not defined.
...
* *Safe dereference*: Expressions `user.name` where `user` is null will throw `NullPointerException` in the language.
  In contrast changular will silently ignore `null` on `user`. This is done because Views often have to wait for the data
...
* *Formatters*: changular expressions can be piped through formatters to further transform the binding value.
  (See: Formatters)
...
Examples of where statements can be used in changular are:
```
...
* *Unsafe dereference*: Expressions `user.verify()` where `user` is `null` will throw `NullPointerException` in the
  language as well as in statements. (In contrast to Safe dereference in changular expressions.) While changular protects
...
* *No Formatters*: changular statements can not contain formatters. (Formatters are only useful for data binding)
