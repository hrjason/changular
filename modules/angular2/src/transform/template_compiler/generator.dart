library changular2.transform.template_compiler.generator;

import 'dart:async';

import 'package:analyzer/analyzer.dart';
import 'package:changular2/src/change_detection/parser/lexer.dart' as ng;
import 'package:changular2/src/change_detection/parser/parser.dart' as ng;
import 'package:changular2/src/core/compiler/pipeline/compile_pipeline.dart';
import 'package:changular2/src/core/compiler/pipeline/compile_step.dart';
import 'package:changular2/src/core/compiler/pipeline/property_binding_parser.dart';
import 'package:changular2/src/core/compiler/pipeline/text_interpolation_parser.dart';
import 'package:changular2/src/core/compiler/pipeline/view_splitter.dart';
import 'package:changular2/src/dom/dom_adapter.dart';
import 'package:changular2/src/reflection/reflection.dart';
import 'package:changular2/src/transform/common/asset_reader.dart';
import 'package:changular2/src/transform/common/logging.dart';
import 'package:changular2/src/transform/common/names.dart';
import 'package:changular2/src/transform/common/parser.dart';
import 'package:barback/barback.dart';

import 'recording_reflection_capabilities.dart';

/// Reads the `.ng_deps.dart` file represented by `entryPoint` and parses any
/// changular 2 `Template` annotations it declares to generate `getter`s,
    ...
    if (node.name is Label && node.name.label is SimpleIdentifier) {
      ...
      if (keyString == 'inline') {
        if (node.expression is SimpleStringLiteral) {
          ...
          logger.error(
              'changular 2 currently only supports string literals in directives',
              ' Source: ${node}');
        }
      }
    } else {
      logger.error(
          'changular 2 currently only supports simple identifiers in directives',
          ' Source: ${node}');
    }
