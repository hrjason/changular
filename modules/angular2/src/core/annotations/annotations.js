import {ABSTRACT, CONST, normalizeBlank, isPresent} from 'changular2/src/facade/lang';
import {ListWrapper, List} from 'changular2/src/facade/collection';
 ...
 * 2. changular instantiates directives for each matched element using [ElementInjector] in a depth-first order,
 ...
 *
 * changular then resolves dependencies as follows, according to the order in which they appear in the [View]:
 *
 ...
 *
 * @publicModule changular2/annotations
 */
@ABSTRACT()
export class Directive extends Injectable {
   ...
   *
   * changular only allows directives to trigger on CSS selectors that do not cross element boundaries.
   *
   ...
   * with standard changular syntax. For example:
   *
   * ```
   * @Decorator({
   *   selector: '[tooltip]',
   ...
   * ```
   *
   * Whenever the `someExpression` expression changes, the `bind` declaration instructs changular to update the
   ...
   * - `$target`: The source of the event. This will be either a DOM element or an changular directive.
   *    [TO BE IMPLEMENTED]
   *
   *
   * ## Syntax
...
/**
 * Declare template views for an changular application.
 *
 * Each changular component requires a single `@Component` and at least one `@Template` annotation. This allows changular to
 ...
 *
 * When a component is instantiated, changular
...
export class Component extends Directive {
   ...
   * injector.
   *
   * When a component is instantiated, changular creates a new child Injector, which is configured with the bindings in
 ...
 *
 * Regular changular components are statically resolved. DynamicComponent allows to you resolve a component at runtime
 * instead by providing a placeholder into which a regular changular component can be dynamically loaded. Once loaded,
 ...
 *
 * There is nothing special about `HelloCmp`, which is a regular changular component. It can also be used in other static
 * locations.
 *
 * ```
 * @DynamicComponent({
 ...
 *
 * Since viewport directives are common in changular, and using the full `<template>` element syntax is wordy, changular
